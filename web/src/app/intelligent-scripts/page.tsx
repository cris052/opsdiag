...
export default function IntelligentScriptsPage() {
  ...
  const [resourceValue, setResourceValue] = useState<Record<string, unknown>>({});
  // 等待状态
  const [isWaiting, setIsWaiting] = useState(false);

  // 为ChatInputPanel提供Context值
  const mockContextValue = {
    scrollRef: { current: null },
    replyLoading: false,
    handleChat: async () => {},
    appInfo: {
      app_code: 'intelligent-scripts',
      layout: {
        chat_in_layout: [
          {
            param_type: 'resource',
    ...
    setResourceValue: (value: any) => {
      ...
      if (value && typeof value === 'object' && 'file_name' in value) {
        setUploadedFileName(value.file_name as string);
      }
    },
    history: [],
    canAbort: false,
    currentDialogue: {} as any,
    maxNewTokensValue: 1000,
    faultTypeValue: '',
    projectNameValue: '',
  ...
  return (
    <ChatContentContext.Provider value={mockContextValue}>
      <style jsx global>{`
        :root{
            ...
            <div className="diagnosis-section">
              ...
              <div className="diagnosis-content">
                <div className="scrollable-diagnosis" ref={diagnosisScrollRef}>
                  {!hasSent ? (
                    <div style={{textAlign: 'center', opacity: 0.7, padding: '20px'}}>请在下方输入后点击发送以开始诊断</div>
            ...
            <div className="log-section">
              ...
              <div className="log-content">
                <div className="scrollable-log" ref={logScrollRef}>
                  {!hasSent ? (
                    <div style={{textAlign: 'center', opacity: 0.7, padding: '20px'}}>请在左侧发送后开始解析</div>
                  ) : isWaiting ? (
