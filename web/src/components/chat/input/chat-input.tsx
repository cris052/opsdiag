...
function ChatInput(prosp: propsT) {
  ...
  return (
    ...
    >
      ...
      <Button
        type='primary'
        className={cls('flex items-center justify-center w-20 h-10 rounded-xl text-sm font-semibold shadow-lg hover:shadow-xl transition-all duration-300 border-0', {
          'bg-gradient-to-r from-violet-500 via-cyan-500 to-emerald-500 hover:from-violet-600 hover:via-cyan-600 hover:to-emerald-600 hover:scale-105 text-white': userInput.trim(),
          'bg-gradient-to-r from-gray-300 to-gray-400 text-gray-500 cursor-not-allowed': !userInput.trim(),
        })}
        onClick={() => {
          if (!userInput.trim()) {
            return;
