...
from opsdiag.model.cluster.registry import EmbeddedModelRegistry, ModelRegistry
from opsdiag.model.parameter import DBModelRegistryParameters, ModelControllerParameters
from opsdiag.util.api_utils import APIMixin
from opsdiag.util.api_utils import _api_remote as api_remote
from opsdiag.util.api_utils import _sync_api_remote as sync_api_remote
from opsdiag.util.fastapi import create_app
from opsdiag.util.tracer.tracer_impl import (
    TracerParameters,
    initialize_tracer,
    root_tracer,
)
...
async def api_register_instance(request: ModelInstance):
    with root_tracer.start_span(
        "opsdiag.model.controller.register_instance", metadata=request.to_dict()
    ):
...
async def api_deregister_instance(
    ...
    with root_tracer.start_span(
        "opsdiag.model.controller.deregister_instance", metadata=instance.to_dict()
    ):
...
def _create_registry(controller_params: ModelControllerParameters) -> ModelRegistry:
    ...
    if not controller_params.registry:
        ...
        from opsdiag.datasource.rdbms.base import (
            RDBMSConnector,
            RDBMSDatasourceParameters,
        )
        from opsdiag.model.cluster.registry_impl.storage import StorageModelRegistry
...
def run_model_controller(config_file: str):
    from opsdiag.configs.model_config import LOGDIR, ROOT_PATH
