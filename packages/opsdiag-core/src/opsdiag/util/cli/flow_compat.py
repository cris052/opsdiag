...
@click.option(
    ...
    required=False,
    help=_(
        "The module to scan, if not set, will scan all OPSDIAG modules("
        "'opsdiag,opsdiag_client,opsdiag_ext,opsdiag_serve,opsdiag_app')."
    ),
)
@click.option(
    "--output",
...
@click.option(
    ...
    required=False,
    help=_(
        "The current version of the flow, if not set, will read from opsdiag.__version__"
    ),
)
@click.option(
