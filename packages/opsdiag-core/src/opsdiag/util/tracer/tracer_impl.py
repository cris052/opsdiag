...
from opsdiag.util.tracer.base import (
    Span,
    SpanStorage,
    SpanStorageType,
    SpanType,
...
from opsdiag.util.tracer.span_storage import MemorySpanStorage
from opsdiag_serve.rag.tracer.rag_flow_span import RagFlowSpanStorage
...
class TracerParameters(BaseParameters):
    file: Optional[str] = field(
        ...
        metadata={
            "help": _(
                "The file to store the tracer, e.g. opsdiag_webserver_tracer.jsonl"
            ),
        },
    )
...
def initialize_tracer(
    tracer_filename: str,
    root_operation_name: str = "opsdiag-Webserver",
    ...
    from opsdiag.util.tracer.span_storage import FileSpanStorage, SpanStorageContainer
    ...
    if tracer_parameters and tracer_parameters.exporter == "telemetry":
        from opsdiag.util.tracer.opentelemetry import OpenTelemetrySpanStorage
    ...
    if system_app.app:
        from opsdiag.util.tracer.tracer_middleware import TraceIDMiddleware
