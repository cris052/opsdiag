...
from opsdiag.configs import VARIABLES_SCOPE_FLOW_PRIVATE
from opsdiag.core.awel import BaseOperator, DAGVar, MapOperator
        ...
        parameters=[
            Parameter.build_from(
                ...
                ui=ui.UIPasswordInput(
                    key="opsdiag.model.openai.api_key",
                ),
            ),
            Parameter.build_from(
                "Model",
                ...
                ui=ui.UIVariablesInput(
                    key="opsdiag.model.openai.model",
                ),
            ),
            Parameter.build_from(
                "DAG Var 1",
                ...
                options=VariablesDynamicOptions(),
                ui=ui.UIVariablesInput(
                    key="opsdiag.core.flow.params", scope=VARIABLES_SCOPE_FLOW_PRIVATE
                ),
            ),
            Parameter.build_from(
                "DAG Var 2",
...
def json_flow():
    ...
    name_to_parameters_dict = {
        "my_test_variables_operator": {
            "openai_api_key": "${opsdiag.model.openai.api_key:my_key@global}",
            "model": "${opsdiag.model.openai.model:default_model@global}",
            "dag_var1": "${opsdiag.core.flow.params:name1@%s}"
            % VARIABLES_SCOPE_FLOW_PRIVATE,
            "dag_var2": "${opsdiag.core.flow.params:name2@%s}"
            % VARIABLES_SCOPE_FLOW_PRIVATE,
        }
    }
        ...
        (
            {
                "vars": {
                    "openai_api_key": {
                        "key": "${opsdiag.model.openai.api_key:my_key@global}",
                        "value": "my_openai_api_key",
                        "value_type": "str",
                        "category": "secret",
                    },
                    "model": {
                        "key": "${opsdiag.model.openai.model:default_model@global}",
                        "value": "GPT-4o",
                        "value_type": "str",
                    },
                }
