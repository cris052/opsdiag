...
from opsdiag.core.awel.util.parameter_util import (
    BaseDynamicOptions,
    OptionValue,
    RefreshOptionRequest,
)
from opsdiag.core.interface.serialization import Serializable
from opsdiag.util.executor_utils import DefaultExecutorFactory, blocking_func_to_async
from opsdiag.util.i18n_utils import LazyTranslatedString
...
class BaseMetadata(BaseResource):
    ...
    documentation_url: Optional[str] = Field(
        ...
        examples=["https://docs.opsdiag.site/docs/awel"],
    )
...
class ResourceMetadata(BaseMetadata, TypeMetadata):
    ...
    parent_cls: List[str] = Field(
        ...
        examples=[
            "opsdiag.core.interface.llm.LLMClient",
            "resource_derisk.model.proxy.llms.chatgpt.OpenAILLMClient",
        ],
    )
...
def auto_register_resource(
    ...
    **decorator_kwargs,
    ...
    from opsdiag.util.function_utils import TYPE_STRING_TO_TYPE
...
class ViewMetadata(BaseMetadata):
    ...
    type_cls: Optional[str] = Field(
        ...
        examples=["opsdiag.model.operators.LLMOperator"],
    )
