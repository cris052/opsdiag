...
class BaseOperator(DAGNode, ABC, Generic[OUT], metaclass=BaseOperatorMeta):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not runner:
            from opsdiag.core.awel import DefaultWorkflowRunner
    ...
    async def call(
        ...
        with root_tracer.start_span("opsdiag.awel.operator.call"):
    ...
    def _blocking_call(
        ...
        Returns:
            OUT: The output of the node after execution.
        """
        from opsdiag.util.logger import get_or_create_event_loop
    ...
    async def call_stream(
        ...
        if call_data != EMPTY_DATA:
            call_data = {"data": call_data}
        with root_tracer.start_span("opsdiag.awel.operator.call_stream"):
            out_ctx = await self._runner.execute_workflow(
                self,
                call_data,
                streaming_call=True,
            ...
            return root_tracer.wrapper_async_stream(
                stream_generator, "opsdiag.awel.operator.call_stream.iterate"
            )
