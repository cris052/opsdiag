...
from opsdiag.core.interface.parameter import BaseServerParameters
from opsdiag.datasource.parameter import BaseDatasourceParameters
from opsdiag.datasource.rdbms.base import RDBMSConnector, RDBMSDatasourceParameters
from opsdiag_app.config import ApplicationConfig, ServiceConfig
...
def _migration_db_storage(
    ...
    from opsdiag.configs.model_config import PILOT_PATH
    ...
    if not disable_alembic_upgrade:
        from opsdiag.storage.metadata.db_manager import db
        from opsdiag.util._db_migration_utils import _ddl_init_and_upgrade
        ...
        if (
            ...
            init and upgrade. "
                ...
                < ./assets/schema/opsdiag.sql` "
                "2.If there are any changes to the table columns in the DERISK database,
                 it is necessary to compare with the DERISK/assets/schema/opsdiag.sql file
...
def _initialize_db(
    ...
    from opsdiag.configs.model_config import PILOT_PATH
    from opsdiag.storage.metadata.db_manager import initialize_db
    from opsdiag_ext.datasource.rdbms.dialect.oceanbase.ob_dialect import (  # noqa: F401
        OBDialect,
    )
    ...
    if system_app:
        from opsdiag.storage.metadata import UnifiedDBManagerFactory
