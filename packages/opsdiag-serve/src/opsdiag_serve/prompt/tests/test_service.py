...
from opsdiag.storage.metadata import db
from opsdiag_serve.core.tests.conftest import system_app  # noqa: F401
    ...
    [
        {
            "app_config": {
                "DEBUG": True,
                "opsdiag.serve.prompt.default_user": "derisk",
                "opsdiag.serve.prompt.default_sys_code": "derisk",
            }
        }
    ],
    indirect=True,
...
def test_config_default_user(service: Service):
    system_app = service._system_app
    assert system_app.config.get("DEBUG") is True
    assert system_app.config.get("opsdiag.serve.prompt.default_user") == "derisk"
    assert service.config is not None
